...
import {
  ...
  generateContentTopicBrief,
  getContentTopicBrief,
  getContentTopicOverview,
  type ContentTopicOverviewDTO
} from "@/lib/api";
...
export default function ContentTopicOverviewPage() {
  ...
  const [overview, setOverview] = useState<ContentTopicOverviewDTO | null>(null);
  ...
  const itemState = useItemState({ targetType: "content_topic", targetKey: `${orgUnitId}:${topicId}` });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const payload = await getContentTopicOverview({ orgUnitId, topicId });
        if (cancelled) return;
        setOverview(payload);
      } catch (error) {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const cached = await getContentTopicBrief({ orgUnitId, topicId });
        if (cancelled) return;
        setAiBrief(cached);
      } catch (error) {
        if (cancelled) return;
  ...
  }, [aiBrief, isGenerating, itemState, orgUnitId, overview, topicId]);

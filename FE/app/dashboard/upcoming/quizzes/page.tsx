    ...
    <UpcomingPageShell>
      {({ events, isLoading }) => {
        ...
        const grouped = useMemo(() => {
          const filtered = filterQuizzes(events);
        ...
        if (grouped.length === 0) return <UpcomingEmptyState itemType="quizzes" />;
            ...
            {grouped.map((group) => (
              <UpcomingTimeGroup key={group.key} label={group.label} count={group.events.length}>
                {group.events.map((event) => {
                  ...
                  if (startAt && endAt) {
                    dateLabel = `Window: ${format(startAt, "EEE, MMM d 'at' p")} \u2013 ${format(endAt, "EEE, MMM d 'at' p")}`;
                  } else if (startAt) {
                    dateLabel = `Due: ${format(startAt, "EEE, MMM d 'at' p")}`;
                  } else {
                    dateLabel = "Due: TBD";
                  }
